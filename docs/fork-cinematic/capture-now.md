...
2. Start a new OBS recording for the complete comparison. If the test has not advanced any rounds, continue from that same round-zero session. Show the initial mode and values for 5 seconds. Run `/fork power clinic`. Run `/fork advance` once, wait for its committed round, then repeat until **round 6**. Keep the waiting and any failure in the source. Do not issue another advance while a round is pending.
