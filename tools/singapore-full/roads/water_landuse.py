...
def _prepare_edges(polygons):
    """Cache inclusive Z bounds; retain original ring and edge ordering."""
    return tuple(tuple(tuple((a, b, min(a[1], b[1])-EPS, max(a[1], b[1])+EPS)
                             for a, b in zip(ring, ring[1:])) for ring in polygon)
                 for polygon in polygons)

def _row_edges(prepared, z):
    # Outside this inclusive range an edge cannot satisfy either _on_segment
    # or the strict ray-crossing predicate. Retained edges use unchanged math.
    rows = []
    for polygon in prepared:
        outer = tuple((a,b) for a,b,z0,z1 in polygon[0] if z0 <= z <= z1)
        if outer:
            holes = tuple(tuple((a,b) for a,b,z0,z1 in ring if z0 <= z <= z1)
                          for ring in polygon[1:])
            rows.append((outer, *holes))
    return tuple(rows)

def _inside_edges(x, z, edges):
    inside = False
    for a, b in edges:
        if _on_segment(x, z, a, b):
            return True
...
def _contains_edges(x, z, polygons):
    return any(_inside_edges(x, z, polygon[0])
               and not any(_inside_edges(x, z, hole) for hole in polygon[1:])
               for polygon in polygons)
...
def rasterize(features, core, clip_mask=None, *, max_candidates=1_000_000):
    ...
    if clip_mask is None:
        ...
        prepared_mask = _prepare_edges(mask_geometry)
        mask = None
    ...
    for polygons, cell, min_x, max_x, min_z, max_z in windows:
        edges = _prepare_edges(polygons)
        for z in range(min_z, max_z+1):
            zc = z+0.5
            row = _row_edges(edges, zc)
            if not row:
                continue
            if prepared_mask is not None and z not in mask_rows:
                mask_rows[z] = _row_edges(prepared_mask, zc)
            for x in range(min_x, max_x+1):
                xc = x+0.5
                in_mask = (_contains_edges(xc, zc, mask_rows[z]) if prepared_mask is not None
                           else mask(xc, zc))
                if in_mask and _contains_edges(xc, zc, row):
                    cells[x, z] = cell
