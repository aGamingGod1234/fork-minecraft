...
const revision=process.argv[2]||'cbd';
if(!['cbd','film-v3'].includes(revision))throw Error('Unknown locator revision');
const outputRelative=`media/source/locator-${revision}-1080p.png`;
...
const args=['--headless=new','--no-first-run','--disable-background-networking','--disable-extensions','--hide-scrollbars','--window-size=1920,1080','--force-device-scale-factor=1','--virtual-time-budget=1000',`--user-data-dir=${path.join(root,'.work/fork/cinematic/chrome-locator-isolated')}`,`--screenshot=${output}`,pathToFileURL(path.join(root,`media/edit/locator-${revision}.html`)).href];
const start=Date.now(),result=runProcess(chrome,args,{timeout:30000});
const evidence={utc:new Date().toISOString(),kind:'Actual attributed SVG rendered by existing Chrome headless, explicitly authorized',command:{executable:chrome,args},exit:result.status,error:result.error?.message,seconds:(Date.now()-start)/1000,output:outputRelative,sha256:fs.existsSync(output)?crypto.createHash('sha256').update(fs.readFileSync(output)).digest('hex'):null,stderr:result.stderr,visuallyReviewed:false,syntheticGameplay:false};
fs.writeFileSync(path.join(root,`.work/fork/cinematic/locator-${revision}-chrome-render.json`),JSON.stringify(evidence,null,2));
