...
export class ForkRunner {
  constructor(service, send, { budgetMs=20000 }={}) { this.service=service;this.send=send;this.budgetMs=budgetMs;this.active=null;this.receipts=new Map(); }
  ...
  async request(raw) {
    ...
    try {
      ...
      if(current()) await this.send('fork_batch',{ticket:p.ticket,intents});
    } catch(error) { if(this.active===a) await this.send('fork_batch',{ticket:p.ticket,error:String(error.message).slice(0,256)}); }
