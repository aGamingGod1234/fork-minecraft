...
public final class ForkFilmClient {
    ...
    public static void register(){
        ClientCommandRegistrationCallback.EVENT.register((dispatcher,registry)->dispatcher.register(ClientCommands.literal("camera").then(ClientCommands.literal("film")
            ...
            .then(ClientCommands.literal("take").executes(c->{if(sequence.active()){notice("Wait for LIVE preparation, or stop it first.");return 0;}source=c.getSource();try{ForkProductCapture.start();return 1;}catch(Exception e){notice(e.getMessage());return 0;}}))
